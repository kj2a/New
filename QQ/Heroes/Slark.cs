...
namespace CarrySharp.Heroes
{
    ...
    class Slark : Vars, IHeroEvents
    {
		...
		{
            Common.Print("Slark by Mhoska, edited by VickTheRock and beminee.");
            AppDomain.CurrentDomain.DomainUnload += CurrentDomain_DomainUnload;
            Menu.AddItem(new MenuItem("enabled", "Enabled").SetValue(true));
            Menu.AddItem(new MenuItem("orbwalk", "orbwalk").SetValue(true));
            //Menu.AddItem(new MenuItem("dodge", "Try to dodge disables by Q").SetValue(true));
            ...
            line = new Line(Drawing.Direct3DDevice9);
                ...
                public async void Combo()
                {
                    ...
                    var invisModif = Common.InvisibleHero(me);



                    Activated = Game.IsKeyDown(Menu.Item("keyBind").GetValue<KeyBind>().Key);
                   // var dodge = Menu.Item("dodge").IsActive();
                        ...
                        if (e != null && e.IsAlive && !e.IsInvul() && !e.IsIllusion)
                        {
                            if (me.CanCast() && !me.IsChanneling())
                            {
                                ...
                                if (Q != null
                                    ...
                                    && me.Distance2D(e) <= 325
                                    && Utils.SleepCheck("Q") && Utils.SleepCheck("QDodge"))
                                {
                                    Q.UseAbility();
                                    Utils.Sleep(200, "Q");
                                }
                    ...
                    /* if (dodge)
                     {
                         var enemies = ObjectManager.GetEntities<Hero>().Where(z => z.Team != me.Team && z.IsAlive && z.IsVisible && !z.IsIllusion && me.Distance2D(e) <= 800).ToList();
                         var disables = Common.Disables;
                         ...
                         foreach (var enemy in enemies)
                         {
                             foreach (var disable in disables)
                             {
                                 ...
                                 var ability = enemy.FindSpell(disable);
                                 if (ability.IsInAbilityPhase)
                                 {
                                     var AbilityTarget = ObjectManager.GetEntities<Hero>()
                             .Where(x => GetDistance2D(x, enemy) <= ability.GetCastRange())
                             .OrderBy(x => Common.RadiansToFace(enemy, x))
                             .FirstOrDefault();

                                     if (AbilityTarget == me)

                                         if (Q.Cooldown >= 0 && Q.ManaCost < me.Mana && Utils.SleepCheck("Q") && Utils.SleepCheck("QDodge"))
                                             me.Spellbook.SpellQ.UseAbility();
                                     Utils.Sleep(150, "QDodge");
                                 }
